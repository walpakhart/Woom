<script lang="ts">
  // Canvas (whiteboard) column — through M-canvas-2.
  //
  // M1 baseline (already shipped):
  //   - Workbench column shell, header, tab strip, "+ new" button.
  ...
  interface Props {
     ...
     *  Lives in `+page.svelte` because per-source navigation
     *  (jira-pane / GH-focus / editor / agent-column) reaches into
     *  state and helpers that page already owns. Optional so the
     *  column still renders standalone in tests. */
  ...
  function onDragOver(e: DragEvent) {
    /* Accept on dragover when the payload is something we can handle:
       ...
       workbench bar) and other host-shell drags pass through cleanly. */
...
<section
  class="wb-column wb-column--canvas"
...
>
  <ColumnControls {instanceId} kind="canvas" />
  <div
    class="wb-col-resize"
  ...
  <header class="canvas-head">
    <div class="canvas-brand">
      <span class="source-mark" aria-hidden="true">
        <svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="14" rx="2"/><rect x="6" y="6" width="9" height="6" rx="1"/><rect x="13" y="13" width="5" height="3" rx="0.5"/></svg>
      </span>
    ...
    <div class="tab-strip" role="tablist">
      {#if instState && instState.tabs.length > 0}
        {#each instState.tabs as tabId (tabId)}
          {@const c = canvasState.open[tabId]}
          {#if c}
            <div
              class="tab"
              class:active={tabId === activeCanvasId}
              role="tab"
              tabindex="0"
              aria-selected={tabId === activeCanvasId}
              onclick={() => onSelectTab(tabId)}
              ondblclick={() => startTabRename(tabId, c.name)}
              onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onSelectTab(tabId); } }}
            ...
            >
              {#if editingTabId === tabId}
                <input
                  ...
                  oninput={(e) => (editingTabDraft = (e.currentTarget as HTMLInputElement).value)}
              ...
              {:else}
                ...
                <button
                  ...
                  title="Close (canvas stays in the library)"
                >
        ...
        {/each}
      {/if}
      <button class="tab-add" onclick={onCreateNew} title="New canvas">
...
<style>
  .wb-column--canvas {
    ...
    /* Library overlay (`.cv-library`) is `position: absolute; inset: 0`
       so a positioned ancestor anchors it to this column's box. */
    position: relative;
  }
  ...
  .library-btn:hover { background: var(--bg-2); color: var(--text-0); border-color: var(--border-neutral); }

  /* ---- Tab strip ---------------------------------------------------- */

  .tab-strip { display: flex; align-items: center; gap: 4px; overflow-x: auto; flex: 1; min-width: 0; scrollbar-width: none; }
  ...
  .tab:hover { background: var(--bg-2); color: var(--text-0); }
  .tab.active { background: var(--bg-2); border-color: var(--border-neutral-hi); color: var(--text-0); }
  .tab-name { overflow: hidden; text-overflow: ellipsis; max-width: 140px; }
  .tab-rename { background: var(--bg-0); border: 1px solid var(--accent); color: var(--text-0); border-radius: 4px; padding: 2px 4px; font-size: 12px; width: 140px; outline: none; }
  .tab-close { width: 14px; height: 14px; border: 0; padding: 0; background: transparent; color: var(--text-2); display: inline-flex; align-items: center; justify-content: center; border-radius: 3px; cursor: pointer; opacity: 0.6; }
  .tab-close:hover { background: var(--bg-0); color: var(--text-0); opacity: 1; }
  .tab-add { width: 22px; height: 22px; flex: 0 0 22px; border-radius: 6px; border: 1px dashed var(--border-neutral-hi); background: transparent; color: var(--text-2); display: inline-flex; align-items: center; justify-content: center; cursor: pointer; }
  .tab-add:hover { border-color: var(--accent); color: var(--accent); background: var(--bg-2); }
  .tab-add svg { width: 12px; height: 12px; }
  ...
  .cv-marquee {
    ...
    background: rgba(238, 107, 31, 0.08);
    pointer-events: none;
