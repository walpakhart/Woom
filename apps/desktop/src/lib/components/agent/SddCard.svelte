<script lang="ts">
   ...
   *  phase with a follow-up prompt that lists each deviation + asks the
   *  agent to address them. Goes through the same auto-fire dispatcher
   *  as the normal three-call advance, so the next agent turn picks up
   *  automatically. */
  ...
  /** Quick-skip without the inline reason textarea. Stamps a default
   *  "user clicked Skip & continue" reason so the audit trail still
   *  has an entry, then flips the phase to skipped and lets the
   *  workflow advance. */
  ...
  /* Audit overlay + its state/lifecycle moved to ./SddAuditOverlay.svelte
   * (wave-1 phase-8 split). We retain only the `open` toggle (so the
   * header chip can flip it) + the `count` bind (for the chip's "·
   * N audit · view" label). The overlay handles refresh, filter,
   * expand, copy, Esc-close internally. */
...
<style>
  ...
  /* Audit overlay styles moved to ./SddAuditOverlay.svelte
   * (wave-1 phase-8 split). The header chip's `.sdd-audit-chip`
   * styles stay here because the chip itself lives in the SddCard
   * header. */
</style>
