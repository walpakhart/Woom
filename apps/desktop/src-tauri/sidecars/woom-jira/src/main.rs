//! woom-jira — MCP sidecar exposing Jira Cloud as tools to Claude Code.
//!
...
const USER_AGENT: &str = concat!("woom-jira/", env!("CARGO_PKG_VERSION"));
...
impl Jira {
    ...
    #[tool(
        description = "List Agile boards. Optional `project_key` to filter to one project; omit to get all accessible (Agile API caps at 50). Useful for resolving board ids before driving sprint creation/start/close (Woom's UI has these but the MCP tool surface is read-only on boards for now)."
    )]
